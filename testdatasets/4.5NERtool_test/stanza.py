...
def NER(sentencelist): #General NER
    ...
    for t in sentencelist:
        sentenceNLP = nlp(t)
        NER+=[(ent.text,ent.type) for ent in sentenceNLP.ents]
    ...
    for t in sentencelist:
        ...
        for ent in sentenceNLP.ents:
            if ent.type == 'ORG':
                ...
                if ent.text[:4]=='the ' or ent.text[:4]=='The ' : 
                    if 'Grant' == ent.text[-1]:
                ...
                elif ent.text[:2]=='a ' or ent.text[:2]=='A ' : 
                    if 'Grant' == ent.text[-1]:
                
                        ORG+=[(ent.text[2:].split('Grant')[0].strip(),ent.type)]
                    else:
                        ORG+=[(ent.text[2:],ent.type)]
