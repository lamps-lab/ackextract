...
def grobid(pdfname):    #input PDFfilename output XMLfilename
    if pdfname+'.tei' in os.listdir():
        print("TEI file already exists in the current directory")
        return pdfname+'.tei'
...
def NER(sentencelist):   # extract all acknowledged entities
    return entityRelationFilter.NERs(sentencelist)
...
def orgNER(sentencelist):   # extract org entities
    ...
    for x,i in enumerate(ORG):
        if 'Grant' == i[0][-5:]:     
            ORG[x] = i[0].split('Grant')[0].strip()
            ORG[x] = (ORG[x],'ORG')
            
    return ORG

def orgNER_Classifier(orgNER_results):
    def org_Classifier(orgNER_result):
        org={'national':['national institutes of health','us national institutes of health','national institute of health','u.s. national institutes of health','nih','national institute of allergy & infectious diseases','niaid','national institute of allergy and infectious diseases','wellcome trust','national science foundation','nsf','public health service','phs','who','cdc','centers for disease control & prevention','centers for disease control and prevention','us centers for disease control & prevention','national multiple sclerosis society','nmss','national cancer institute','nci','bill & melinda gates foundation','bill and melinda gates foundation','national institute of general medical sciences','nigms','usda','ohio agricultural research & development center','ohio agricultural research and development center','oardc','national center for research resources','ncrr','defense threat reduction agency','national pork board','howard hughes medical institute','hhmi','world bank','fogarty international center','american cancer society','acs','mrc','medical reserve corp','american heart association','aha','national center for advancing translational sciences','ncats','morris animal foundation','chest'], 
        'other':['national natural science foundation of china','national science foundation of china','national nature science foundation of china','nsfc','deutsche forschungsgemeinschaft','german research foundation','dfg','canadian institutes of health research','canadian institutes for health research','cihr','european commission','european union','swiss national science foundation','natural sciences & engineering research council of canada','nserc','welfare of japan','national research foundation of korea','japan society for the promotion of science','jsps','ministry of science & technology of china','italian ministry of health','national key research & development program of china','european research council','erc','priority academic program development of jiangsu higher education institutions','chinese academy of sciences','cas','uk medical research council','medical research council','mrc','instituto de salud carlos iii','isciii','science foundation ireland','royal society','national basic research program of china','national science council of taiwan','china scholarship council','indian council of medical research','academia sinica']}
        if orgNER_result[0].lower() in org['national']:
            return (orgNER_result[0],1)
        elif orgNER_result[0].lower() in org['other']:
            return (orgNER_result[0],3)
        elif 'university' in (orgNER_result[0]).lower():
            return (orgNER_result[0],2)
        else:
            return (orgNER_result[0],3)
    
    return list(map(org_Classifier,orgNER_results))
